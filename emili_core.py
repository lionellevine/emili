...
# Create TTS client only when needed so missing API keys do not crash startup.
def get_tts_client():
    global client
...
# messages to be sent to OpenAI API. Outgoing messages only.
message_queue = queue.Queue()
# user has entered a new chat, triggers OpenAI API call
...
# set when there is a sudden change in user emotions
emotion_change_event = threading.Event()
# triggered when the user enters 'q' to end the session
end_session_event = threading.Event()

# for vision API call
user_snapshot_caption = "Camera snapshot of user and surroundings, for context"
...
# for exponential moving average, discount factor per second
discount_factor_per_second = 0.5
# discount factor per tick
discount_factor_per_tick = discount_factor_per_second ** (tick_interval / 1000)
# default 1.0. Higher reactivity means more frequent API calls when emotions change
reactivity = 0.5
# threshold for significant change in emotion scores: C*(1-delta)*sqrt(t). The factor of 1-delta is because EMAs are compared, not raw scores.
ect_setpoint = (1e6/reactivity) * (1.0-discount_factor_per_tick) * \
    ((tick_interval/1000) ** 0.5)
# discount factor for the emotion change threshold
ect_discount_factor_per_second = 0.95
ect_discount_factor_per_tick = ect_discount_factor_per_second ** (
    tick_interval / 1000)

emotion_matrix = []  # shape (7,6)
salience_threshold = []
emotion_matrix.append(["", "Annoyed", "Pissed", "Angry",
                      "Furious", "Enraged"])  # anger
# salience thresholds out of 100
salience_threshold.append([5, 30, 40, 60, 80])
emotion_matrix.append(["", "Unsatisfied", "Displeased",
                      "Disgusted", "Revolted", "Totally grossed out"])  # disgust
salience_threshold.append([1, 5, 15, 40, 60])
emotion_matrix.append(
    ["", "Uneasy", "Worried", "Anxious", "Fearful", "Terrified"])  # fear
salience_threshold.append([8, 20, 30, 50, 70])
emotion_matrix.append(["", "Contented", "Pleased", "Happy",
                      "Elated", "Ecstatic"])  # happiness
salience_threshold.append([10, 30, 40, 70, 90])
emotion_matrix.append(["", "Down", "Melancholy", "Sad",
                      "Despondent", "Anguished"])  # sadness
salience_threshold.append([5, 20, 30, 60, 80])
emotion_matrix.append(["", "Mildly surprised", "Surprised",
                      "Taken aback", "Astonished", "Flabbergasted"])  # surprise
salience_threshold.append([3, 20, 35, 50, 70])
emotion_matrix.append(["", "Neutral", "Calm", "Relaxed",
                      "Serene", "Totally Zen"])  # neutral
...
# system message to remind the assistant
system_reminder = "Remember, the assistant can ask the user to act out a specific emotion!"
dialogue_start = [{"role": "system", "content": system_prompt}]


# Convert numpy array image to base64 to pass to the OpenAI API
def encode_base64(image, timestamp, save_path):
...
# prepends emotion data and current video frame to user input
def assembler_thread(start_time, snapshot_path, pipeline):
    ...
    while not end_session_event.is_set():
        ...
        # note: this code repeated in timer_thread
        ...
        if current_frame is not None:  # capture a frame and send it to the API
            base64_image, filename = encode_base64(
                current_frame, time_since(start_time), snapshot_path)
            message_with_image, brief_message = construct_message_with_image(
                base64_image, filename)
            vision_queue.put([{"role": "system", "content": message_with_image}, {
                             "role": "system", "content": brief_message}])
...
def sender_thread(model_name, vision_model_name, secondary_model_name, max_context_length, gui_app, transcript_path, start_time_str):
    ...
    while not end_session_event.is_set():
        # Wait for a new message to be prepared by the assembler or timer thread
        ...
        while not message_queue.empty():  # get all new messages
            ...
            if next_message[0]["role"] == "user":
                new_user_chat = True
        messages, full_transcript = add_message(
            new_messages, [messages, full_transcript], gui_app.signal)
        ...
        if vision is not None:
            # contains the actual image, send to OpenAI
            vision_message = vision[0]
            # contains a tag in place of the image, add to transcript
            brief_vision_message = vision[1]
            query = messages + [vision_message]
            full_response = get_response(query, model=vision_model_name, temperature=1.0,
                                         max_tokens=max_tokens, seed=1331, return_full_response=True)
            full_transcript.append(brief_vision_message)
        else:
            full_response = get_response(messages, model=model_name, temperature=1.0,
                                         max_tokens=max_tokens, seed=1331, return_full_response=True)
        ...
        if isinstance(full_response, dict):
            ...
            response_length = full_response['usage']['completion_tokens']
            # total tokens used
            total_length = full_response['usage']['total_tokens']
        else:
            # text of response
            response = full_response.choices[0].message.content
            # number of tokens in the response
        ...
        # Signal GUI to display the new chat
        gui_app.signal.new_message.emit(new_message)
        messages, full_transcript = add_message(
            [[new_message]], [messages, full_transcript], gui_app.signal)
        ...
        if total_length > 0.9*max_context_length:  # condense the transcript
            if verbose:
                print(
                    f"(Transcript length {total_length} tokens out of {max_context_length} maximum. Condensing...)")
        ...
        if use_tts:  # generate audio from the assistant's response
            ...
            if tts_client is not None:
                tts_response = tts_client.audio.speech.create(
                    model="tts-1",
                    # alloy (okay), echo (sucks), fable (nice, Australian?), onyx (sucks), nova (decent, a little too cheerful), shimmer (meh)
                    voice="nova",
...
def first_sentence(text):
    # .+ for at least one character, ? for non-greedy (stop at first match), [.!?]+ for one or more punctuation marks, followed by a space
    match = re.search('(.+?[.!?]+) ', text)
    if match:
        # return the first sentence (first match of what's in parentheses)
...
def play_audio():
    pygame.mixer.init()
    # todo: sometimes overwritten by new audio! It just switches in this case, which seems okay.
    pygame.mixer.music.load("tts_audio/tts.mp3")
    pygame.mixer.music.play()


# append one or messages to both transcripts
def add_message(new_messages, transcripts, signal):
    ...
    for msg in new_messages:  # len(msg)=1 for text, 2 for text and image
        ...
        # sent to OpenAI: contains the base64 image if present
        transcripts[0].append(msg[0])
        # recorded in full_transcript: contains only the image filename
        transcripts[1].append(msg[-1])
    # Signal GUI transcript tab to update
    signal.update_transcript.emit(transcripts[1])
    return transcripts
...
# todo: reduce total number of tokens to below 16k
def condense(messages, keep_first=1, keep_last=5):
    condensed = []
    N = len(messages)  # number of messages
    previous_message = {}
    # remove system messages except for the last few
...
# calculates the exponential moving average of the emotion logs
def EMA_thread(start_time, snapshot_path, pipeline):
    ...
    while not end_session_event.is_set():
        ...
        # exponential moving average of the emotion logs
        ...
        if ema is not None:
            ...
            diff = ema - last_ema
            # Euclidean norm. todo add weights for different emotions
            ...
            if (change > ect and time_since(last_emotion_change_time) > 5000):
                ...
                print(
                    f"Change in emotions: {last_ema//1e4} -> {ema//1e4}, change = {change//1e4}")
                # bool evaluates to True if the inequality holds
                change_detected = (change > 0.5*ect_setpoint)
                emolog_message = construct_emolog_message(change_detected)
                message_queue.put(
                    [{"role": "system", "content": emolog_message}])
                current_frame = pipeline.current_frame
                if current_frame is not None:  # capture a frame and send it to the API
                    base64_image, filename = encode_base64(
                        pipeline.current_frame, time_since(start_time), snapshot_path)
                    message_with_image, brief_message = construct_message_with_image(
                        base64_image, filename)
                    vision_queue.put([{"role": "system", "content": message_with_image}, {
                                     "role": "system", "content": brief_message}])
...
# calculates the exponential moving average of the emotion logs
def get_average_scores(S, Z, discount_factor=discount_factor_per_tick, staleness_threshold=0.01):
...
# add camera frame to the message for gpt-4-vision
def construct_message_with_image(base64_image, filename, caption=user_snapshot_caption, detail_level="low", change_detected=False):
...
def construct_emolog_line(emo_scores):
    ...
    if emo_scores is not None:
        ...
        normalized_scores = np.array(
            emo_scores//1e4, dtype=int)  # convert to 0-100
        # returns salience score of 0-5 for each of 7 emotions
        emotion, salience = adjust_for_salience(normalized_scores)
        sorted_indices = np.argsort(normalized_scores)[
            ::-1]  # descending order
        emotion[sorted_indices[0]] = emotion[sorted_indices[0]
                                             ].upper()  # strongest emotion in uppercase
...
# expects 7 scores normalized to 0-100
def adjust_for_salience(normalized_scores):
    salience = []
    emotion = []
    ...
    # emotion is a string (empty if salience is 0); salience is 0-5
    return emotion, salience
...
class Emolog:  # video pipeline for facial emotion recognition using SigLIP2
    ...
    # siglip index -> emili index (-1 = discard)
    SIGLIP_TO_EMILI = [-1, 0, 3, 6, 4, 5]
    EMILI_LABELS = ['anger', 'disgust', 'fear',
                    'happiness', 'sadness', 'surprise', 'neutral']
    # Labels shown to the user in the video overlay.
    DISPLAY_LABELS = ['angry', 'disgust', 'fear',
                      'happy', 'sad', 'surprised', 'neutral']
    # 5-class label space used by personalization training/evaluation.
    TARGET_EMOTIONS = ['angry', 'happy', 'neutral', 'sad', 'surprised']
    TARGET_SIGLIP_INDICES = [1, 2, 3, 4, 5]
    # Map internal labels to personalization target labels.
    EMILI_TO_TARGET = {
        'anger': 'angry',
        'happiness': 'happy',
        'sadness': 'sad',
    ...
    def __init__(self, start_time, offsets=None, personalized_checkpoint=None, intensity_path=None):
        ...
        # Load optional per-user checkpoint from personalization.py.
        if personalized_checkpoint is not None and os.path.exists(personalized_checkpoint):
            ...
            if isinstance(payload, dict) and 'model_state_dict' in payload:
                ...
                print(
                    f"Loaded personalized FER checkpoint from {personalized_checkpoint}")
            else:
                print(
                    f"Warning: invalid personalized checkpoint format at {personalized_checkpoint}")
        self.model = self.model.to(self.device).eval()
        # Load optional per-user intensity calibration map.
        if intensity_path is not None and os.path.exists(intensity_path):
            ...
            print(
                f"Loaded personalized intensity calibrator from {intensity_path}")
    ...
    def call(self, image):
        ...
        if len(faces) > 0:
            ...
            if h > 150:
                ...
                # Intensity is separate from confidence; neutral returns N/A.
                intensity = self._predict_intensity(
                    dominant_label, target_probs)
    ...
    def _classify(self, pil_crop):
        inputs = self.processor(
            images=pil_crop, return_tensors="pt").to(self.device)
        with torch.no_grad():
            logits = self.model(**inputs).logits
        # Original model probabilities over SigLIP2 output classes.
        siglip_probs = torch.softmax(logits, dim=-1).squeeze().cpu().numpy()
        # Personalization probabilities over the 5 target classes.
        target_probs = torch.softmax(
            logits[:, self.TARGET_SIGLIP_INDICES], dim=-1).squeeze().cpu().numpy()
        ...
        if total > 0:
            # Re-normalize after dropping Ahegao from the mapped score vector.
            emili_scores /= total
    ...
    def _predict_intensity(self, dominant_label, target_probs):
        ...
        # Stronger intensity signal: confidence + margin to second-best class.
        ...
        if self.intensity_calibrator is not None and target_label in self.intensity_calibrator:
            ...
            if coeff.get('mode') == 'bounded_linear':
                ...
                # User-specific bounded map keeps intensity stable in [1, 5].
