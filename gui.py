...
class VideoPlayerWorker(QObject):
    ...
    def step(self):
        ...
        # shape: [height, width, 3], dtype: uint8. Macbook camera height=720, width=1280
        frame = self.camera.read()
        if frame is None:
            print('No camera input.')
            return None
        frame = convert_color_space(frame, BGR2RGB)
        # FER pipeline returns a dictionary with keys 'image' and 'boxes2D' (bounding boxes for faces)
    ...
    def run(self):  # this is where the main thread ends up living its lonely life
        ...
        while not self.stop_flag:
            ...
            if image is None:
                continue
            # image is a numpy array of shape [width,height,3] and dtype uint8
        ...
        stats.strip_dirs().sort_stats('cumulative').print_stats(
            10)  # Adjust as needed to view more or fewer lines
...
class DisplaySignal(QObject):
    # Signal to display fresh emotion scores, carries list payload with time-series of emotion scores
    fresh_scores = pyqtSignal(list)
...
class Visualizer(QMainWindow):  # GUI for real-time FER visualizer
    def __init__(self, start_time, dims, colors, speed, pipeline, end_session_event, camera_id=0):
        ...
        # expects an np array of shape (7,3) representing an RGB color for each basic emotion
        self.colors = colors
...
#        self.central_widget.setStyleSheet("background-color: black;")
        ...
        # redraw the display in the visualizer tab
        self.signal.fresh_scores.connect(self.redraw_visualizer)

        self.timer = QTimer(self)
        self.timer.timeout.connect(self.redraw_visualizer)
        self.timer.start(40)  # calls redraw_visualizer every 40 ms

        # timout to prevent frequent window resizes when user is dragging the window
    ...
    def init_FER_tab(self):
        ...
        self.FER_image.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)  # allow user to resize
    ...
    def init_visualizer_tab(self):
        ...
        self.visualizer_image.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)  # allow user to resize
    ...
    def enterFullScreen(self):
        # don't show window and tab titles in fullscreen mode
        self.setWindowFlags(
            Qt.Window | Qt.FramelessWindowHint | Qt.CustomizeWindowHint)
        self.showFullScreen()
        self.is_full_screen = True
        # Delay focus setting to ensure transition completion
        QTimer.singleShot(100, self.setFocusToActiveTab)
#            self.tab_widget.setFocus()
#            self.resizeTimer.start()

    def exitFullScreen(self):
        # show window and tab titles in normal mode
    ...
    # Assuming tab_widget is the QTabWidget and each tab is a QWidget
    def setFocusToActiveTab(self):
        current_tab = self.tab_widget.currentWidget()
        if current_tab is not None:
            current_tab.setFocus()

    # expects a list of [time, scores] pairs in chronological order
    def redraw_visualizer(self):
        ...
        # get the most recent binned time series
        binned_time_series = self.pipeline.binned_time_series[-self.num_bins:]
        ...
        # most recent bin end time
        bin_end_time = binned_time_series[-1][0] + self.interval
        while bin_end_time < current_time:  # catch up to current_time
            # print("(redraw_visualizer) catching up, empty bin")
            # pad data with most recent scores
            binned_time_series.append([bin_end_time, last_scores])
            bin_end_time += self.interval

        # draw the tunnel
        image = np.zeros(
            (self.display_height, self.display_width, 3), dtype=np.uint8)
        # draw the most recent scores first
        for timestamp, scores in reversed(binned_time_series):
            # print("(redraw_visualizer) item",item)
            # timestamp = item[0]
            # scores = item[1]
            # if(len(item)>2):
            #     print("(redraw_visuzlizer) long item! item[2], len(item)",item[2],len(item))
            # most recent data at center, 25 pixels per second
            radius = (current_time - timestamp)//self.interval
            ...
            if (x_min < 0 and y_min < 0):  # draw partial rectangles too
                break
            # matrix multiplication (3,7) @ (7,1) = (3,1)
            combined_color = self.colors.T @ (scores/1e6)
            ...
            # corner, corner, color, thickness
            image = draw_rectangle(image, (x_min, y_min),
                                   (x_max, y_max), combined_color.tolist(), 1)
        ...
        qImg = QImage(image.data, self.display_width,
                      self.display_height, bytesPerLine, QImage.Format_RGB888)
        pixmap = QPixmap.fromImage(qImg)
        # pixmap will be displayed in the Visualizer tab of the GUI
...
class ChatSignal(QObject):
    # Signal to display a new user message, carries dict payload with message
    new_message = pyqtSignal(dict)
    # Signal to update the transcript display, carries list payload with transcript
    update_transcript = pyqtSignal(list)
...
class ChatApp(QMainWindow):  # GUI for LLM video chat
    def __init__(self, start_time, chat_window_dims, user_chat_name, assistant_chat_name, chat_queue, chat_timestamps, new_chat_event, end_session_event):
        ...
        # Set the height to accommodate three lines of text
        self.chat_input.setFixedHeight(72)
        # Adjust the height and font-size as needed
        self.chat_input.setStyleSheet(
            "QLineEdit { height: 80px; font-size: 24px; }")
        # function to call when user presses Enter
        self.chat_input.returnPressed.connect(self.act_on_user_input)
    ...
    def act_on_user_input(self):
        ...
        if user_input:
            # Signal chat pane to display user message
            self.signal.new_message.emit(
                {"role": "user", "content": user_input})
            self.chat_input.clear()
            # milliseconds since start of session
            self.chat_timestamps.put(time_since(self.start_time))
            # pass user message to the assembler thread
    ...
    def init_transcript_tab(self):
        ...
        # Set the layout for the transcript tab
        self.transcript_tab.setLayout(layout)
        # Add the transcript tab to the main tab widget
    ...
    def update_transcript_display(self, full_transcript):
        ...
        transcript_json = json.dumps(
            full_transcript, indent=4, sort_keys=False)
        # Replace escaped newlines with actual newlines
        transcript_json = transcript_json.replace('\\n', '\n')
        # Save the current scroll position
        scroll_position = self.transcript_display.verticalScrollBar().value()
        self.transcript_display.setPlainText(
            transcript_json)  # renders as plain text, no HTML
        self.transcript_display.verticalScrollBar().setValue(
            scroll_position)  # Restore the scroll position
